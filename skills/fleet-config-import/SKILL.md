...
6. **Agents** (match-by-name, idempotent)
   ...
   - Match existing by name when present; leave their UUIDs local. Skip create when the name already exists.
   - **Routines** (match-by-name, idempotent): for each new or existing bot, create routine **recipes** from pack only when no routine with that **name** already exists on the bot (`update_state` routine create: name, scheduleOrTriggerIntent, prompt). Never invent automation.json ids. Never delete existing routines to match the pack.

7. **Rooms** (match-by-name, idempotent)
   - Match existing channels by **name**. If a room with that name already exists, skip `CreateChannel` (idempotent intent); refresh membership only when the operator asks.
   - For each missing `rooms[]`: `CreateChannel` by name/charter; members from `memberRoleKeys` resolved to live agent ids (max **6** members; include self if managing the channel).
...
9. **Journal**
   - Append a short note under `$FLEET_DIR` (or herd-journal style) listing: plugins installed, rules written, skills written, agents created (names/roleKeys), rooms created vs skipped-by-name, routines requested vs skipped-by-name, healthcheck status.
...
- Never delete rooms or routines to force pack parity
